...
- **Acquired Asset 1**: **Plug-and-Play Anomaly Detection Engine** (Seller: Voltiq / Ashmit-Roy, Agreed Price: ₹4.00 Cr FED Coins)
- **Acquired Asset 2**: **AccessRoute Delivery & RBAC Engine** (Seller: Prabh707, Agreed Price: ₹4.20 Cr FED Coins)
- **Sold Asset**: **Universal Low-Connectivity Sync Layer** (Buyer Team, Sale Price: ₹4.80 Cr FED Coins)
- **Deal Register Compliance**: Recorded at the SEBI Registration Desk.

### ⚡ Slide 3 — What We Integrated (Technical Integration & Defense)
1. **🚨 Anomaly Detection Engine**:
   - Integrated in [`src/acquired_modules/anomaly-detection-engine`](https://github.com/Suvrodipto/apna-dukan/tree/main/src/acquired_modules/anomaly-detection-engine) & [`AcquiredAnomalyPage.tsx`](https://github.com/Suvrodipto/apna-dukan/blob/main/src/components/acquired/AcquiredAnomalyPage.tsx).
   - Calculates statistical Z-score dispersion ($Z = \frac{x - \mu}{\sigma}$) flagging POS checkout surges and triggering VIP Gold loyalty & ₹5,000+ Festive Gift Hampers.
2. **🗺️ AccessRoute Delivery & RBAC Engine**:
   - Integrated in [`src/acquired_modules/access-route`](https://github.com/Suvrodipto/apna-dukan/tree/main/src/acquired_modules/access-route) & [`AcquiredAccessRoutePage.tsx`](https://github.com/Suvrodipto/apna-dukan/blob/main/src/components/acquired/AcquiredAccessRoutePage.tsx).
   - Calculates shortest TSP delivery routes between Kirana hub, suppliers, and customer home deliveries using Haversine distance equations ($O(V^2)$), saving fuel costs (₹/km) and enforcing role-based permissions (Owner, Cashier, Supplier, Delivery).
